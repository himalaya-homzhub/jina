...
@pytest.mark.parametrize('local_ip, on_public', [(get_internal_ip(), False),
                                                 (get_public_ip(), True)])
def test_remote_pod_local_pod_local_gateway_force_pull_socket_from_remote(local_ip, on_public):
    remote_ip = '111.111.111.111'
    f = Flow(expose_public=on_public, force_pull_socket_from_remote=True).add(host=remote_ip).add().build()
    for k, v in f:
        print(f'{v.name}\tIN: {v.address_in}\t{v.address_out}')
    assert f['pod0'].host_in == __default_host__
    assert f['pod0'].host_out == __default_host__
...
@pytest.mark.parametrize('local_ip, on_public', [(get_internal_ip(), False),
                                                 (get_public_ip(), True)])
def test_remote_pod_local_pod_remote_pod_local_gateway_force_pull_socket_from_remote(local_ip, on_public):
    remote1 = '111.111.111.111'
    remote2 = '222.222.222.222'

    f = Flow(expose_public=on_public, force_pull_socket_from_remote=True).add(host=remote1).add().add(
        host=remote2).build()
    for k, v in f:
        print(f'{v.name}\tIN: {v.address_in}\t{v.address_out}')
...
def test_gateway_remote_local():
    ...
    assert f['pod0'].host_in == __default_host__
    assert f['pod0'].host_out == get_internal_ip()
    assert f['pod0'].args.socket_in == SocketType.PULL_BIND
    assert f['pod0'].args.socket_out == SocketType.PUSH_CONNECT

    assert f['pod1'].host_in == __default_host__
    assert f['pod1'].host_out == __default_host__
    assert f['pod1'].args.socket_in == SocketType.PULL_BIND
...
def test_gateway_remote_local_force_pull_socket_from_remote():
    """
    ...
    """
    remote1 = '111.111.111.111'
    f = Flow(force_pull_socket_from_remote=True).add(host=remote1).add().build()
    for k, v in f:
        print(f'{v.name}\tIN: {v.address_in}\t{v.address_out}')
