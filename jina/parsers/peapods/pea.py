...
def mixin_pea_parser(parser):
    ...
    gp.add_argument('--force-pull-socket-from-remote', action='store_true', default=False,
                    help='If set, in the case where a Pod is local with respect to the Flow, it forces the incoming '
                         'socket connection from remote instances to be of type PULL. This is useful when the Pod is '
                         'not directly reachable from remote and therefore it needs to PULL.')
